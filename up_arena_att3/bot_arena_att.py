...
# --- SISTEMAS DE COMBATE SEPARADOS ---

def combate_normal():
    """Ataque padrão usando apenas skills do 1 ao 4"""
...
def gerenciar_combate_full():
    """Lógica completa com BM e Aura (Para o Boss Final)"""
    ...
    if bm_ativa:
        ...
        # Tenta ativar BM se tiver SP e não estiver em cooldown
        try:
            r, g, b = pyautogui.pixel(SP_BOLINHA_X, SP_BOLINHA_Y)
...
def ciclo_arena(num):
    ...
    time.sleep(0.3)
    keyboard.press_and_release(DASH_1); time.sleep(1.2)
    ...
    # 3. Seleção e Entrada
    pyautogui.click(ARENAS[num]); time.sleep(1.5)
    ...
    while time.time() - inicio_t < 5:
        ...
        if verificar_imagem('indisponivel', conf=0.7):
            keyboard.press_and_release("esc"); return "PROXIMA_ARENA"
        time.sleep(0.5)
    ...
    # Passo 9: Portão (MELHORIA: SEM BM AQUI)
    print("[PORTÃO] Quebrando portão com Skills Normais...")
    keyboard.press_and_release(ABA_MOVIMENTO); time.sleep(0.3)
    ...
    while rodando:
        combate_normal() # <--- Aqui usamos apenas skills 1-4
    ...
    # Boss Final (MELHORIA: BM E AURA AQUI)
    print("[BOSS] Iniciando combate final com BM...")
    keyboard.press_and_release(ABA_COMBATE); time.sleep(0.3)
    while not verificar_imagem('botao_ok') and rodando:
        gerenciar_combate_full() # <--- Aqui ele tenta ativar BM e Aura
...
def fluxo_principal():
    ...
    while rodando:
        ...
        print("[SISTEMA] Ciclo concluído. Reiniciando...")
        time.sleep(5)
